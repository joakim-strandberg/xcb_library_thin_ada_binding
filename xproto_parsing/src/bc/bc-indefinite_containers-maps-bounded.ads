...
--  $Revision: 1458 $
--  $Date: 2011-03-11 22:32:11 +0000 (Fri, 11 Mar 2011) $
--  $Author: simonjwright $

with BC.Support.Indefinite_Bounded;
with BC.Support.Indefinite_Bounded_Hash_Tables;

generic
   with function Hash (K : Key) return Natural is <>;
   Buckets : Positive;
   Maximum_Size : Positive;
package BC.Indefinite_Containers.Maps.Bounded is
...
private
   ...
   package KC is new BC.Support.Indefinite_Bounded
     (Item => Key,
      "=" => Maps."=",
      Item_Ptr => Key_Ptr);
   package Keys is new BC.Support.Indefinite_Bounded_Hash_Tables.Item_Signature
     (Item => Key,
      Item_Ptr => Key_Ptr,
      Eq =>  Maps."=",
      Hash => Hash);

   package IC is new BC.Support.Indefinite_Bounded
     (Item => Item,
      "=" => Indefinite_Containers."=",
      Item_Ptr => Item_Ptr);
   package Items
   is new BC.Support.Indefinite_Bounded_Hash_Tables.Value_Signature
     (Value => Item,
      Value_Ptr => Item_Ptr,
      Eq => Indefinite_Containers."=");

   package Tables is new BC.Support.Indefinite_Bounded_Hash_Tables.Tables
     (Items => Keys,
      Values => Items);
...
end BC.Indefinite_Containers.Maps.Bounded;
